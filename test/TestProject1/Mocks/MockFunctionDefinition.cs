...
public class MockFunctionDefinition : FunctionDefinition
{
    ...
    public MockFunctionDefinition(IDictionary<string, BindingMetadata> inputBindings)
    {
    ...
    public static FunctionDefinition Generate(int inputBindingCount = 0, int outputBindingCount = 0, params Type[] paramTypes)
    {
        ...
        inputs.Add($"triggerName", new MockBindingMetadata("httpTrigger", BindingDirection.In, "req"));

        for (var i = 0; i < inputBindingCount; i++)
        {
            inputs.Add($"inputName{i}", new MockBindingMetadata($"TestInput{i}", BindingDirection.In, $"test{i}"));
        }

        for (var i = 0; i < outputBindingCount; i++)
        {
            outputs.Add($"outputName{i}", new MockBindingMetadata($"TestOutput{i}", BindingDirection.Out, $"test{i}"));
        }
